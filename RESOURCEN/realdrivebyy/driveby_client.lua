...
addEventHandler("doSendDriveBySettings",localPlayer,
	function(newSettings)
		...
		local newTable = {}
		for key,vehicleID in pairs(settings.blockedVehicles) do
...
function toggleDriveby()
	...
	if getElementData ( localPlayer, "isInRace" ) then return end
	...
	if equipedWeapon == 0 then
		if isingw then return false end
		...
		if not lastSlotAmmo or lastSlotAmmo == 0 or getSlotFromWeapon(getPedWeapon (localPlayer,lastSlot)) == 0 then
			for key,weaponID in pairs(weaponsTable) do
			...
			for key,weaponID in pairs(weaponsTable) do --If our last used weapon is a valid weapon
