						...
						if not isOnStateDuty ( driver ) and getElementModel ( hit ) ~= 438 then
							if not isMedic ( driver ) then
								...
								if speed > 100 * 0.00464 then
									...
									if speed > 100 * 0.00464 then
										if not vioGetElementData ( player, "highscore_achiev" ) then
											...
											dbExec ( handler, "UPDATE ?? SET ?? = ? WHERE ?? = ?", "achievments", "highscore", "1", "UID", playerUID[getPlayerName ( player )] )
										end
									end
									
									vioSetElementData ( player, "bankmoney", vioGetElementData ( driver, "bankmoney" ) - strafe )
									
									triggerClientEvent ( player, "createNewStatementEntry", player, "Strafzahlung\n", strafe*-1, math.abs ( math.floor ( kmhToMuch ) ).." km/h zu\nschnell gefahren" )
									
									vioSetElementData ( player, "stvo_punkte", points + vioGetElementData ( player, "stvo_punkte" ) )
