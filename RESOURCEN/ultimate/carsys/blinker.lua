...
function blinkerbinds(vehinplayer)
	...
	if (getVehicleType ( blinkercar ) == "Automobile") and (seat == 0) then
		...
		bindKey ( vehinplayer, "num_1", "up", linkerblinker)
		bindKey ( vehinplayer, "num_3", "up", rechterblinker)
		bindKey ( vehinplayer, "num_5", "up", warnblinker)
...
function blinkerunbinds(vehinplayer)
	...
	unbindKey ( theVehPlayer, "num_1", "up", linkerblinker)
	unbindKey ( theVehPlayer, "num_3", "up", rechterblinker)
	unbindKey ( theVehPlayer, "num_5", "up", warnblinker)
...
--[[function togglelicht( vehinplayer )
	for index, vehtable in pairs(blinkertable) do
		for index2, wert in pairs(vehtable) do
			if (index2== "Player") then
				 BlinkerPlayertl = wert
			elseif (index2== "Car") then
				 BlinkerCartl = wert
			elseif (index2== "BlinkerStatus") then
				 BlinkerStatustl = wert
			elseif (index2== "BremslichtStatus") then
				 BremslichtStatustl = wert
			elseif (index2== "Lichtstate") then
				 Lichtstatetl = wert
			end
		end
		if (getPedOccupiedVehicle ( vehinplayer ) == BlinkerCartl)  and (Lichtstatetl == 0) and (BlinkerStatustl == 0) then 
			vehtable.Lichtstate = 1
			setVehicleLightState ( BlinkerCar, 1, 0)
			setVehicleLightState ( BlinkerCar, 0, 0)
		elseif (getPedOccupiedVehicle ( vehinplayer ) == BlinkerCartl)  and (Lichtstatetl == 1) and (BlinkerStatustl == 0) then 
			vehtable.Lichtstate = 0
			setVehicleLightState ( BlinkerCar, 1, 1)
			setVehicleLightState ( BlinkerCar, 0, 1)	
		-- Rausgenommen, bis 1.0.4 draußen ist !!
		--elseif (getPedOccupiedVehicle ( vehinplayer ) == BlinkerCartl)  and (Lichtstatetl == 2) and (BlinkerStatustl == 0) then 
		--	vehtable.Lichtstate = 0
		--	setVehicleLightState ( BlinkerCar, 1, 0)
		--	setVehicleLightState ( BlinkerCar, 0, 0)
		end
...
function toggleVehicleLights ( player, key, state )
	if not(getVehicleType ( getPedOccupiedVehicle ( player ) ) == "Automobile") then
		...
		if ( getPedOccupiedVehicleSeat ( player ) == 0 ) then
			local veh = getPedOccupiedVehicle ( player )
			if ( vioGetElementData ( veh, "light") ~= true ) then	
				vioSetElementData ( veh, "light", true)
			else	
				vioSetElementData ( veh, "light", false)
			end
