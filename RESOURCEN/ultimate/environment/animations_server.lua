...
function sex_func(player)
	if vioGetElementData ( player, "jailtime" ) > 0 or vioGetElementData ( player, "prison" ) > 0 or isPedInVehicle ( player ) then
		return
	end
...
function handsup(Player)
	if vioGetElementData ( Player, "jailtime" ) and  vioGetElementData ( Player, "jailtime" ) > 0 or vioGetElementData ( Player, "prison" ) and vioGetElementData ( Player, "prison" ) > 0 or isPedInVehicle ( Player ) then
		return
	end
	vioSetElementData(Player,"anim", 1)
...
function smoke(Player)
	if vioGetElementData ( Player, "jailtime" ) and vioGetElementData ( Player, "jailtime" ) > 0 or vioGetElementData ( Player, "prison" ) and vioGetElementData ( Player, "prison" ) > 0 then
		return
	end
	if not gotLastHit[Player] or gotLastHit[Player] + healafterdmgtime <= getTickCount() then
		if vioGetElementData ( Player, "zigaretten" ) and vioGetElementData ( Player, "zigaretten" ) >= 1 then
...
function eat(Player,cmd,slot)
	if vioGetElementData ( Player, "jailtime" ) > 0 or vioGetElementData ( Player, "prison" ) > 0 then
		return
	end
	if slot then
