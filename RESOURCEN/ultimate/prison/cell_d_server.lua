...
function Zell_Function_D ( mouseButton, buttonState, ele )
	...
	if buttonState == "up" then
		...
		if not vioGetElementData( ele, "objectCellInfoD" ) or vioGetElementData( ele, "objectCellInfoD" ) == nil then
			return
		end
		
		local number = tonumber(vioGetElementData( ele, "objectCellInfoD" ))
