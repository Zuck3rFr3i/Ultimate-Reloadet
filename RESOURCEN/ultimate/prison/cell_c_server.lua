...
function Zell_Function_C ( mouseButton, buttonState, ele )
	...
	if buttonState == "up" then
		...
		if not vioGetElementData( ele, "objectCellInfoC" ) or vioGetElementData( ele, "objectCellInfoC" ) == nil then
			return
		end
		
		local number = tonumber(vioGetElementData( ele, "objectCellInfoC" ))
