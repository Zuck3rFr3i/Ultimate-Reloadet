...
for i, obj in pairs ( schleusen_keypads ) do
	...
	vioSetElementData ( obj, "SchleusenNummer", count )
...
function outputClick ( mouseButton, buttonState, ele )
	...
	if buttonState == "up" then
			...
			if not vioGetElementData( ele, "SchleusenNummer" ) or vioGetElementData( ele, "SchleusenNummer" ) == nil then
				return
			end
			...
			local schleuse = tonumber(vioGetElementData( ele, "SchleusenNummer" ))
