...
function Zell_Function_B ( mouseButton, buttonState, ele )
	...
	if buttonState == "up" then
		...
		if not vioGetElementData( ele, "objectCellInfoB" ) or vioGetElementData( ele, "objectCellInfoB" ) == nil then
			return
		end
		
		local number = tonumber(vioGetElementData( ele, "objectCellInfoB" ))
