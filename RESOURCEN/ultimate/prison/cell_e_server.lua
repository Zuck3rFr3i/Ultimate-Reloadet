...
function Zell_Function_E ( mouseButton, buttonState, ele )
	...
	if buttonState == "up" then
		...
		if not vioGetElementData( ele, "objectCellInfoE" ) or vioGetElementData( ele, "objectCellInfoE" ) == nil then
			return
		end
		
		local number = tonumber(vioGetElementData( ele, "objectCellInfoE" ))
