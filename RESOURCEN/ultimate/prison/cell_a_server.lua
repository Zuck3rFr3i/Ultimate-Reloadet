...
function Zell_Function_A ( mouseButton, buttonState, ele )
	...
	if buttonState == "up" then
		...
		if not vioGetElementData( ele, "objectCellInfoA" ) or vioGetElementData( ele, "objectCellInfoA" ) == nil then
			return
		end
		
		local number = tonumber(vioGetElementData( ele, "objectCellInfoA" ))
