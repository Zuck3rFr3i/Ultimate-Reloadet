...
function exchangeWeaponsWithBox_func ( player, slot, action )
	...
	if player == client then
		...
		if isOnDuty(player) then
			...
			if action == "ex" then
				local slot1 = vioGetElementData ( player, "gunboxa" )
				local slot2 = vioGetElementData ( player, "gunboxb" )
				local slot3 = vioGetElementData ( player, "gunboxc" )
			...
			elseif action == "in" then
				if slot == 1 then
					vioSetElementData ( player, "gunboxa", curgun.."|"..curammo )
				elseif slot == 2 then
					vioSetElementData ( player, "gunboxb", curgun.."|"..curammo )
				else
					vioSetElementData ( player, "gunboxc", curgun.."|"..curammo )
				end
