...
function changeFireMode ()
	...
	if fireModeWeapons[getPedWeapon ( lp )] and not getPedOccupiedVehicle ( lp ) then
		if firestate == "auto" then
			if ( getElementData ( lp, "fraktion" ) == 8 and vioClientGetElementData ( "rang" ) >= 1 ) or getElementData ( lp, "fraktion" ) ~= 8 then
				firestate = "burst"
			elseif getElementData ( lp, "fraktion" ) == 8 and vioClientGetElementData ( "rang" ) == 0 then
				firestate = "semi"
			end
		...
		-- firestate == "semi"
			if getElementData ( lp, "fraktion" ) == 8 and vioClientGetElementData ( "rang" ) >= 1 then
				firestate = "burst"
...
function bindFireModeChangeKey ()
	...
	if getElementData ( lp, "fraktion" ) == 8 then
		if vioClientGetElementData ( "rang" ) > 1 then
			firestate = "auto"
		elseif vioClientGetElementData ( "rang" ) == 1 then
			firestate = "burst"
		else
			firestate = "semi"
		end
