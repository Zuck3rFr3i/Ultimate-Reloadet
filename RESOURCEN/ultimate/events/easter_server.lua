function createNewEasterEgg ()
	...
	if not isElement ( easterEggsInUse[rnd] ) then
		...
		vioSetElementData ( shape, "id", rnd )
...
function easterEggShapeHit ( hit, dim )
	...
	if getElementType ( hit ) == "player" and dim then
		...
		vioSetElementData ( hit, "easterEggs", vioGetElementData ( hit, "easterEggs" ) + 1 )
...
function buyEasterBonus ( item )
	...
	if item == "suess" then
		if eggs >= 10 then
			vioSetElementData ( player, "easterEggs", eggs - 10 )
	...
	elseif item == "state" then
		if eggs >= 100 then
			vioSetElementData ( player, "easterEggs", eggs - 100 )
			vioSetElementData ( player, "socialState", "Sensenmann" )
	...
	elseif item == "dollar" then
		if eggs >= 250 then
			vioSetElementData ( player, "easterEggs", eggs - 250 )
	...
	elseif item == "gutschein" then
		if eggs >= 1200 then
			vioSetElementData ( player, "everyCarBuyableForFree", true )
			vioSetElementData ( player, "easterEggs", eggs - 1200 )
			...
			if not getPedOccupiedVehicle ( player ) then
				if getFreeCarSlot ( player ) then
					...
					if carbuy ( player, 0, 451, -898.5, 2715.6, 46, 0, 0, 97.5 ) then
						vioSetElementData ( player, "easterEggs", eggs - 2000 )
						outputChatBox ( "Happy Halloween wuenscht dir SERVERNAME!", player, 125, 0, 0 )
					end
			...
			if not getPedOccupiedVehicle ( player ) then
				if getFreeCarSlot ( player ) then
					...
					if carbuy ( player, 0, 503, -898.5, 2715.6, 46, 0, 0, 97.5 ) then
						vioSetElementData ( player, "easterEggs", eggs - 5000 )
						outputChatBox ( "Happy Halloween wuenscht dir SERVERNAME!", player, 125, 0, 0 )
					end
					vioSetElementData ( player, "everyCarBuyableForFree", false )
				else
