	...
	if result and result[1] then
		for i=1, #result do
			...
			if Besitzer then
				...
				vioSetElementData ( vehicle, "color", dsatz["Farbe"] )
				vioSetElementData ( vehicle, "lcolor", dsatz["Lights"] )
				...
				vioSetElementData ( vehicle, "distance", tonumber ( dsatz["Distance"] ) )
				vioSetElementData ( vehicle, "stuning", dsatz["STuning"] )
				vioSetElementData ( vehicle, "spezcolor", dsatz["spezcolor"] )
				setVehiclePlateText( vehicle, dsatz["plate"] )
				vioSetElementData ( vehicle, "rcVehicle", tonumber ( dsatz["rc"] ) )
				vioSetElementData ( vehicle, "sportmotor", tonumber (dsatz["Sportmotor"]) )
				vioSetElementData ( vehicle, "bremse", tonumber (dsatz["Bremse"] ) )
				vioSetElementData ( vehicle, "antrieb", dsatz["Antrieb"] )
				setVehicleLocked ( vehicle, true )
				vioSetElementData ( vehicle, "fuelstate", tonumber ( dsatz["Benzin"] ) )
				setPrivVehCorrectColor ( vehicle )
				setPrivVehCorrectLightColor ( vehicle )
				setVehiclePaintjob ( vehicle, tonumber ( dsatz["Paintjob"] ) )
				if special == 2 then
