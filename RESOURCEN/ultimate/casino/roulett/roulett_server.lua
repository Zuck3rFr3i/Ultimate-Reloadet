...
function startRoulett_func ( string, count )
	...
	local player = client
	local betMoney = 0
	local playerChips = vioGetElementData ( player, "casinoChips" )
	...
	if betMoney <= playerChips then
		if betMoney <= calcPlayerMaxGambleBet ( player, 0 ) then
			vioSetElementData ( player, "casinoChips", playerChips - betMoney )
			...
			local diff = vioGetElementData ( player, "casinoChips" ) - playerChips
			casinoAchievCheck ( player, diff )
			if diff > 0 then
			...
			triggerClientEvent ( player, "showRoulettWinningNumber", player, roulettWinningNumber, vioGetElementData ( player, "casinoChips" ) - playerChips )
		else
...
function roulettWin ( multiply, amount, player )
	...
	vioSetElementData ( player, "casinoChips", vioGetElementData ( player, "casinoChips" ) + multiply * amount )
end