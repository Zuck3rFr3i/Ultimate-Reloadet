...
for key, index in pairs ( blackJackTables["x1"] ) do
	...
	function blackjackMarkerHit ( hit )
		if hit == lp then
			if vioClientGetElementData ( "casinoChips" ) > 0 then
...
function blackJackScrollBarChanged ()
	...
	local money = math.floor ( vioClientGetElementData ( "casinoChips" ) / 100 * guiScrollBarGetScrollPosition ( gScroll["blackJack"] ) + 0.5 )
...
function blackJackEditChanged ()
	...
	if scrollBarChanged then
		...
		if curTotalEditChips ~= guiGetText ( gEdit["blackJack"] ) then
			...
			if money then
				if money <= vioClientGetElementData ( "casinoChips" ) then
					money = math.floor ( money + 0.5 )
					guiSetText ( gEdit["blackJack"], money )
					curTotalEditChips = money
					money = money / vioClientGetElementData ( "casinoChips" ) * 100
					guiScrollBarSetScrollPosition ( gScroll["blackJack"], money )
				else
					guiSetText ( gEdit["blackJack"], vioClientGetElementData ( "casinoChips" ) )
				end
			else
