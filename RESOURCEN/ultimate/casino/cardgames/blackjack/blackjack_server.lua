...
function startBlackJack_func ( amount )
	...
	if player == source then
		if not vioGetElementData ( player, "blackJackStarted" ) then
			vioSetElementData ( player, "blackJackStarted", true )
			...
			if amount <= chips then
				if amount <= calcPlayerMaxGambleBet ( player, 1 ) then
					if amount > 0 then
						vioSetElementData ( player, "curBlackJackBet", amount )
...
function endBlackJackCurGame ( player, lost, blackJack )
	...
	if vioGetElementData ( player, "blackJackStarted" ) then
		vioSetElementData ( player, "blackJackStarted", false )
		local r, g, b, text = 0, 0, 0, ""
		local bet = vioGetElementData ( player, "curBlackJackBet" )
		
		if not lost then
			casinoAchievCheck ( player, bet )
			g = 150
			if blackJack then
			...
			if not blackJack then
				vioSetElementData ( player, "casinoChips", vioGetElementData ( player, "casinoChips" ) + bet * 2 )
			else
				vioSetElementData ( player, "casinoChips", vioGetElementData ( player, "casinoChips" ) + bet * 3 )
			end
		else
			casinoAchievCheck ( player, -1 * bet )
...
function blackJackDouble_func ()
	...
	if i == 6 then
		...
		local chips = vioGetElementData ( player, "casinoChips" )
		if chips >= bet then
			vioSetElementData ( player, "casinoChips", chips - bet )
			vioSetElementData ( player, "curBlackJackBet", bet * 2 )
			local player = client
			local i = blackJackCards[player]["playerCardCount"] + 1
			blackJackCards[player]["playerCardCount"] = i
			blackJackCards[player]["player"][i] = getBlackJackCard ( player )
