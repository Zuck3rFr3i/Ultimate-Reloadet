...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
	...
	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y-(gblurFactor*1))) * 0.8;
	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y+(gblurFactor*1))) * 0.8;

	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y-(gblurFactor*2))) * 0.65;
	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y+(gblurFactor*2))) * 0.65;

	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y-(gblurFactor*3))) * 0.4;
	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y+(gblurFactor*3))) * 0.4;

	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y-(gblurFactor*4))) * 0.25;
	Color += tex2D(Sampler0, float2(PS.TexCoord.x, PS.TexCoord.y+(gblurFactor*4))) * 0.25;
