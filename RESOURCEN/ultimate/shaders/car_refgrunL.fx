...
//for reflection factor look for brightnessFactor in piel shader
  ...
  float alphaFactor = 0.7;
  float minusAlpha = 0.2;
  float dirtTex =1;
  float bumpSize =0.02;
  float sNormZ =3;
  float sRefFl =1;
...
float4 PixelShaderFunction(PSInput PS) : COLOR0
{
   ...
   float3 vView = PS.View;
	...
	vReflection.x +=vNp2.x*bumpSize;	
	vReflection.y +=vNp1.y*(bumpSize/2);
    ...
    envMap+=sAdd+1.0; //brighten
    envMap = (envMap * adj);
    envMap = pow(envMap, sPower+2); // contrast
	envMap*=sMul;
	...
	float3 alpha = (envMap.r+envMap.g+envMap.b)/3.0f;
	Color.a =(alphaFactor-alpha.r);
