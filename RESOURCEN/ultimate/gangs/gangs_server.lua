		...
		if vioGetElementData ( player, "fraktion" ) == 0 then
			if not isInGang ( pname, nil ) then
				...
				if bankmoney >= costsToCreateGang then
					if not vioGetElementData ( player, "gangCreateTry" ) then
						vioSetElementData ( player, "gangCreateTry", true )
						...
						vioSetElementData ( player, "gangCreateTry", false )
						...
						vioSetElementData ( player, "bankmoney", bankmoney - costsToCreateGang )
					end
				else
...
function leavegang_func ( player )
	...
	if isInGang ( pname ) then
		if not isInGang ( pname, vioGetElementData ( player, "housekey" ) ) then
...
function guninvite_func ( player, cmd, name )
	...
	if uninvitedPlayer then
		if isInGang ( uninvitedPlayer, getPlayerGang ( player ) ) then
			if not isInGang ( uninvitedPlayer, vioGetElementData ( uninvitedPlayer, "housekey" ) ) then
		...
		if isInGang ( player ) then
			if not isInGang ( invitedPlayer ) then
				if getPlayerGangRank ( player ) >= 3 then
					...
					if vioGetElementData ( invitedPlayer, "fraktion" ) == 0 then
		...
		if v1 == "money" then
			if take then
				if getGangMoney ( id ) >= v2 then
					...
					vioSetElementData ( player, "money", money + v2 )
		...
		elseif v1 == "drugs" then
			...
			if take then
				if getGangDrugs ( id ) >= v2 then
					vioSetElementData ( player, "drugs", drugs + v2 )
		...
		elseif v1 == "mats" then
			...
			if take then
				if getGangMats ( id ) >= v2 then
					vioSetElementData ( player, "mats", mats + v2 )
