...
function outputPlayerWarns ( name, reader )
	...
	if count > 0 then
		for i = 1, count do
			...
			outputChatBox ( "Von: "..array["admin"].." ( "..array["date"].." ), Grund: "..array["reason"], reader, 255, 0, 0 )
			outputChatBox ( "Ablaufdatum: "..array["extends_o"], reader, 255, 0, 0 )
		end
	end
end
		...
		if suspect then
			...
			if extends and tonumber(extends) ~= nil then
				...
				if extends and extends > 0 and extends <= 365 then
					...
					dbExec ( handler, "INSERT INTO ?? ( ??,??,??,??,??,?? ) VALUES (?,?,?,?,?,?)", "warns", "UID", "adminUID", "reason", "extends", "extends_o", "date", playerUID[name], playerUID[admin], reason, rt.timestamp + 3600 * 24 * extends, dateextendet..", 4:00", date )
