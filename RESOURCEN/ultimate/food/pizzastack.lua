...
function pizzaBuySFCenterHit ( player, dim )
	...
	if dim == true then
		vioSetElementData ( player, "intdim", getElementDimension ( player ) )
		vioSetElementData ( player, "curpizza", 1 )
...
function pizzaLeave ( player )
	...
	setElementDimension ( player, vioGetElementData ( player, "intdim" ) )
...
function pizzaLeft ( player )
	...
	local pizza = tonumber ( vioGetElementData ( player, "curpizza" ) )
	if pizza == 1 then
		pizza = 4
	elseif pizza == 2 then
		pizza = 1
...
function pizzaRight ( player )
	...
	local pizza = vioGetElementData ( player, "curpizza" )
	if pizza == 1 then
		pizza = 2
	elseif pizza == 2 then
		pizza = 3
...
function pizzaLookCheck ( player, pizza )
	...
	vioSetElementData ( player, "curpizza", pizza )
	if pizza == 1 then
		local px, py, pz = getElementPosition ( healthyPizza )
		setCameraMatrix ( player, px-.04, py-0.4, pz+0.5, px, py, pz )
...
function pizzaBuy ( player )
	...
	if pmoney >= pizzaprice then
		...
		vioSetElementData ( player, "money", pmoney - pizzaprice )
