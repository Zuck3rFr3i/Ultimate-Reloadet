...
local museumprice = 5000   -- Falls man nur die ersten 4 einsammelt
local museummainprice = 10000   -- Falls man den 5. bekommt
...
function hitAHostage ( )
	if vioGetElementData ( source, "money" ) >= 300 then
		outputChatBox ( "Verletze nicht die Zivilisten! Dafür werden dir 300$ abgezogen!", source, 255, 0, 0 )
		takePlayerSaveMoney ( source, 300 )
	elseif vioGetElementData ( source, "bankmoney" ) >= 300 then
		outputChatBox ( "Verletze nicht die Zivilisten! Dafür werden dir 300$ von der Bank abgezogen!", source, 255, 0, 0 )
		vioSetElementData ( source, "bankmoney", vioGetElementData ( source, "bankmoney" ) - 300 )
	elseif vioGetElementData ( source, "money" ) + vioGetElementData ( source, "bankmoney" ) >= 300 then
		...
		vioSetElementData ( source, "bankmoney", vioGetElementData ( source, "bankmoney" ) - ( 300 - money ) )
	else
		local money = vioGetElementData ( source, "money" ) + vioGetElementData ( source, "bankmoney" )
		outputChatBox ( "Verletze nicht die Zivilisten! Dafür wird dir all dein restliches Geld weggenommen!", source, 255, 0, 0 )	
		takePlayerSaveMoney ( source, vioGetElementData ( source, "money" ) )
		vioSetElementData ( source, "bankmoney", 0 )
	end
end
...
function goInOutMuseum ( hitElement, dim )
	if getElementType ( hitElement ) == "player" and dim then
		...
		vioSetElementData ( hitElement, "spawnProtection", true )
		setTimer ( vioSetElementData, 1000, 1, hitElement, "spawnProtection", false )
	end
end
...
function startMuseumRob_click ( )
	if robberGangs[vioGetElementData ( source, "fraktion" )] then
...
function startMuseumRob ( player )
	museumrobbing = true 
	robberfaction = vioGetElementData ( player, "fraktion" )
	robberplayer = player
	addEventHandler ( "onPlayerWasted", player, museumRobPlayerWasted )
	addEventHandler ( "onPlayerQuit", player, museumRobPlayerWasted )
	outputLog ( "Museumraub - "..getPlayerName(player).." - "..fraktionNames[vioGetElementData ( player, "fraktion")], "aktion" )
	animpedtyp = 1
	for thePlayer, _ in pairs ( playerInMuseum ) do
...
function museumVehicleEnter ( player, _, jacked, door )
	if vioGetElementData ( player, "fraktion" ) ~= robberfaction then
