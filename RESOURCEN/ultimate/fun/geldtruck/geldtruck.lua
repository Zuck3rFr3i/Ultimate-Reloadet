...
local function gFinish (player)
	...
	if player and isElement ( player) and getElementType(player) == "player" then
		...
		if veh then 
			if veh_seat == 0 then
				if veh == gTruck then  
					outputChatBox("Das Fahrzeug wurde abgegeben, jeder Staatsfraktionist in der Nähe kriegt 5.000$",player, 0, 125, 0)
					...
					for i=1, #players do
						if isOnDuty ( players[i] ) then
							vioSetElementData ( players[i], "money", vioGetElementData ( players[i], "money" ) + 5000 )
						end
					end
					setTimer(gOnDriveRestart,60*1000*60,1)
				end
			...
			if gOnDrive == 0 then
				if not aktionlaeuft then
					...
					if hours >= 12 then
						...
						gOnDrive = 1 
						aktionlaeuft = true
						outputLog ( "Geldtruck - "..getPlayerName(player).." - "..fraktionNames[vioGetElementData ( player, "fraktion")], "aktion" )
