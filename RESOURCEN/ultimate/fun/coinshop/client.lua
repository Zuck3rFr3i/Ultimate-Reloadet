...
addEventHandler ( "updateCoinsInCoinshop", root, function ()
	guiSetText ( shopLabel, "Coins: "..vioClientGetElementData ( "coins" ) )
end )
...
local function buttonOneFunc ( button, state )
	if button == "left" and state == "up" then
		if vioClientGetElementData ( "coins" ) >= cars[1][3] then
...
local function buttonFourFunc ( button, state )
	if button == "left" and state == "up" then
		if vioClientGetElementData ( "coins" ) >= 10 then
...
local function buttonFiveFunc ( button, state )
	if button == "left" and state == "up" then
		if vioClientGetElementData ( "coins" ) >= 300 then
...
local function buttonSixFunc ( button, state )
	if button == "left" and state == "up" then
		if vioClientGetElementData ( "coins" ) >= 200 then
...
addCommandHandler ( "coinshop", function ()
	...
	shopwindow = guiCreateWindow ( width/2-250, height/2-77, 500, 153, "Ultimate-RL Coin-Shop", false )
	...
	shopLabel = guiCreateLabel ( 0.675, 0.75, 0.25, 0.2, "Coins: "..vioClientGetElementData ( "coins" ), true, shopwindow )
