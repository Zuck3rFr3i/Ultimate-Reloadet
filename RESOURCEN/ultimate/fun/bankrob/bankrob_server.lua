...
function exit_bank ( thePlayer, matchingDimension )
	...
	if vioGetElementData (thePlayer, "gotdamoney") and not bereitsdraussen then
		vioSetElementData ( bank_robber, "money", vioGetElementData (bank_robber, "money" ) + 35000 )
...
function info_bank_timer ()
	...
	if bank_i == 0 then
		...
		if isElementWithinColShape ( bank_robber, bank_rob_col ) then
			...
			vioSetElementData (bank_robber, "gotdamoney", true)
			setTimer ( function ( player )
				if vioGetElementData (player, "gotdamoney") then
					if not bereitsdraussen then
						outputChatBox ( "Das Geld hat sich in Luft aufgelöst!", player, 200, 0, 0 )
					end
					vioSetElementData (player, "gotdamoney", nil)
				end
...
function bank_rob ( player, _ )
	...
	if isEvil ( player ) and isElementWithinColShape ( player, bank_rob_col ) then
		...
		if bank_wird_ausgeraubt ~= 1 then
			...
			bank_robber = player
			bereitsdraussen = false
			outputLog ( "Bankraub - "..getPlayerName(player).." - "..fraktionNames[vioGetElementData ( player, "fraktion")], "aktion" )
...
function nehmeGeldAb ( player, cmd, target )
	if isOnDuty ( player ) then
		if target then
			...
			if targetpl then
				if vioGetElementData ( targetpl, "gotdamoney" ) then
					if bereitsdraussen then
						if vioGetElementData ( targetpl, "money" ) >= 35000 then
							vioSetElementData ( targetpl, "money", vioGetElementData ( targetpl, "money" ) - 35000 )
						else 
							vioSetElementData ( targetpl, "money", 0 )
						end
						vioSetElementData (targetpl, "gotdamoney", false)
