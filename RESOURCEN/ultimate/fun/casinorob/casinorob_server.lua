...
addEventHandler("setCasinoPreparationServer", getRootElement(), function()
	if casinoRobber == nil and not aktionlaeuft then
		...
		if int == 1 then
			if casinorobDonecaligulas[tonumber(vioGetElementData(client, "fraktion"))] == nil or casinorobDonecaligulas[tonumber(vioGetElementData(client, "fraktion"))] == false then
			else
				outputChatBox("Unbekannt: Sie haben uns doch heute schon \195\188berfallen, wir haben nichts mehr!", client, 255, 255, 255)
				return false
			end	
		elseif int == 10 then
			if casinorobDonefourdragons[tonumber(vioGetElementData(client, "fraktion"))] == nil or casinorobDonefourdragons[tonumber(vioGetElementData(client, "fraktion"))] == false then
			else
				outputChatBox("Unbekannt: Sie haben uns doch heute schon \195\188berfallen, wir haben nichts mehr!", client, 255, 255, 255)
				return false
			end	
		elseif int == 12 then
			if casinorobDonecasinofloor[tonumber(vioGetElementData(client, "fraktion"))] == nil or casinorobDonecasinofloor[tonumber(vioGetElementData(client, "fraktion"))] == false then
			else
				outputChatBox("Unbekannt: Sie haben uns doch heute schon \195\188berfallen, wir haben nichts mehr!", client, 255, 255, 255)
				return false
			end	
		...
		outputLog ( "Casinorob - "..getPlayerName(client).." - "..fraktionNames[vioGetElementData ( client, "fraktion")], "aktion" )
		casinoRobberFraction = vioGetElementData(client, "fraktion")
		casinoTimerMarker = setTimer(casinoRobEnded, 1000*60*14, 1, 4)
		casinoTimer = setTimer(casinoRobEnded, 1000*60*20, 1, 3)
		local x, y, z = getElementPosition(client)
		local robberFraction = vioGetElementData(client, "fraktion")
		local players = getElementsByType("player")
		for i=1, #players do
			local player = players[i]
			local x1, y1, z1 = getElementPosition(player)
			local int1 = getElementInterior(player)
			if robberFraction == vioGetElementData(player, "fraktion") then
		...
		if int == 1 then
			...
			casinorobDonecaligulas[tonumber(vioGetElementData(client, "fraktion"))] = true
		...
		elseif int == 10 then
			...
			robbedFraction = 3
			casinorobDonefourdragons[tonumber(vioGetElementData(client, "fraktion"))] = true
		...
		elseif int == 12 then
			...
			robbedFraction = 9
			casinorobDonecasinofloor[tonumber(vioGetElementData(client, "fraktion"))] = true
...
function casinorobLootClick_func ( button, state, clickedElement )
	if vioGetElementData(source, "fraktion") ~= casinoRobberFraction then return false end	
						...
						if getPlayerName(source) == player then
							...
							if clickedElement == thevehicle then
								...
								if casinorobErfolg == false then
									...
									-- "Yakuza" --
									if tonumber(vioGetElementData ( casinoRobber, "fraktion" )) == 3 then
										abgabeMarker = createMarker(2815, 955.90002441406, 10.800000190735, "checkpoint", 6, 222, 0, 0, 255)
									-- "La Cosa Nostra" --
									elseif tonumber(vioGetElementData ( casinoRobber, "fraktion" )) == 7 then
										abgabeMarker = createMarker(1041.5999755859, 2112.1999511719, 10.800000190735, "checkpoint", 6, 23, 23, 23)
									-- "Angels of Death" --
									elseif tonumber(vioGetElementData ( casinoRobber, "fraktion" )) == 9 then
...
function casinoRobEnded(success)
	...
	if success ~= 4 then
		...
		local players = getElementsByType("player")
		for i=1, #players do
			if vioGetElementData(players[i], "fraktion") == casinoRobberFraction then
...
function getCasinorobLoot ( player, index )
	...
	elseif randomnumber >= 1 and randomnumber < 750 then
		...
		if itemnumber >= 93 then
			...
			vioSetElementData ( player, "benzinkannister", tonumber ( vioGetElementData ( player, "benzinkannister" ) ) + Benzinkanister )
		elseif itemnumber >= 65 then
			outputChatBox("Box "..tostring(index).." von "..tostring(anzahlafterobjects).." - "..Zigaretten.." Zigaretten", player)
			vioSetElementData ( player, "zigaretten", vioGetElementData ( player, "zigaretten" ) + Zigaretten )
		...
		elseif itemnumber >= 20 then
			...
			vioSetElementData ( player, "mats", vioGetElementData ( player, "mats" ) + Materialien )
		...
		elseif itemnumber >= 1 then
			...
			vioSetElementData ( player, "flowerseeds", seeds + Hanfsamen )
		end
	end
end	
...
function freezeDatShit (player)
	if vioGetElementData(player, "fraktion") == casinoRobberFraction then
