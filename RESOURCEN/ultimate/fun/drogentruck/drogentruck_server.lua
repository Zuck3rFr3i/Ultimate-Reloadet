...
function loadDrogenTruck ( player )
	...
	outputLog ( "Drogentruck - "..getPlayerName(player).." - "..fraktionNames[vioGetElementData ( player, "fraktion")], "aktion" )
	addEventHandler ( "onVehicleExplode", drogentruck, vehicleDestroyedDrogenTruck )
	drogentruckFraktion = vioGetElementData ( player, "fraktion" )
...
function drogentruckSuccessfull ( player )
	...
	outputChatBox ("Der Drogentransporter wurde abgebeben!", player, 0, 255, 0)
	destroyElement (drogentruck)
	setTimer ( function() aktionlaeuft = false end, aktionpuffer, 1)
	setTimer ( function() drogentruck = false end, 60*60*1000, 1 )
	setElementVisibleTo ( drogentruckDeliver, getRootElement(), false )
	...
	vioSetElementData ( player, "flowerseeds", vioGetElementData (player, "flowerseeds") + 50 )
	vioSetElementData ( player, "drugs", vioGetElementData (player, "drugs") + 1500 )
end
