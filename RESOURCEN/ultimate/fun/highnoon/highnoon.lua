...
function highnoon_func ( player, cmd, name, summe, runden )
	...
	if name and name ~= player then
		...
		if summe then
			if summe <= vioGetElementData ( player, "money" ) and vioGetElementData ( name, "money" ) then
				...
				if runden then
					if runden >= 1 and runden <= 10 then
						...
						if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= 5 and getDistanceBetweenPoints3D ( x1, y1, z1, x3, y3, z3 ) <= 5 then
							...
							vioSetElementData ( player, "highNoonQuestion", getPlayerName ( name ) )
							vioSetElementData ( name, "highNoonQuestion", getPlayerName ( player ) )
							vioSetElementData ( name, "highNoonSumme", summe )
							vioSetElementData ( name, "highNoonRunden", runden )
						else
...
function accepthighnoon_func ( player, cmd )
	...
	local name = vioGetElementData ( player, "highNoonQuestion" )
	if name then
		local name = getPlayerFromName ( name )
		if name then
			setElementDimension ( player, 0 )
			setElementDimension ( name, 0 )
			local summe = vioGetElementData ( player, "highNoonSumme" )
			if summe <= vioGetElementData ( player, "money" ) and vioGetElementData ( name, "money" ) then
				if not isPedDead ( player ) and not isPedDead ( name ) and not vioGetElementData ( player, "isInHighNoon" ) and not vioGetElementData ( name, "isInHighNoon" ) then
					removePedFromVehicle ( player )
					removePedFromVehicle ( name )
					if cmd == "accepthighnoon" then
						vioSetElementData ( player, "highNoonWin", 0 )
						vioSetElementData ( name, "highNoonWin", 0 )
					end
					local x1, y1, z1 = getElementPosition ( player )
					local x2, y2, z2 = getElementPosition ( name )
					local x3, y3, z3 = getElementPosition ( highnoonIcon )
					if getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 ) <= 5 and getDistanceBetweenPoints3D ( x1, y1, z1, x3, y3, z3 ) <= 5 then
						local runden = vioGetElementData ( player, "highNoonRunden" )
						...
						local dim = tonumber ( vioGetElementData ( player, "playerid" ) )
						takeAllWeapons ( player )
						setElementDimension ( player, dim )
						setPedArmor ( player, 0 )
						vioSetElementData ( player, "isInHighNoon", true )
						vioSetElementData ( name, "highNoonRunden", runden )
						...
						vioSetElementData ( name, "isInHighNoon", true )
						...
						if cmd == "accepthighnoon" then
							...
							vioSetElementData ( name, "money", vioGetElementData ( name, "money" ) - summe )
						end
					else
						outputChatBox ( "Du und dein Gegner muessen am Waffenladen sein!", player, 125, 0, 0 )
					end
...
function highNoonDraw ( p1, p2 )
	...
	local runden = vioGetElementData ( p1, "highNoonRunden" )
	local summe = vioGetElementData ( p1, "highNoonSumme" )
	if (vioGetElementData ( p1, "isInHighNoon" ) and vioGetElementData ( p2, "isInHighNoon" )) then
		...
		elseif isPedDead ( p1 ) then
			...
			if isElement ( p2 ) then
				...
				vioSetElementData ( p2, "highNoonWin", vioGetElementData ( p2, "highNoonWin" ) + 1 )
		...
		elseif isPedDead ( p2 ) then
			...
			if isElement ( p1 ) then
				...
				vioSetElementData ( p1, "highNoonWin", vioGetElementData ( p1, "highNoonWin" ) + 1 )
...
function checkIfItIsOver ( p1, p2 )
	...
	local runden = vioGetElementData ( p1, "highNoonRunden" ) - 1
	if runden then
		if runden >= 1 then
			...
			vioSetElementData ( p1, "highNoonRunden", runden )
			vioSetElementData ( p2, "highNoonRunden", runden )
			setElementDimension ( p1, 1 )
			setElementDimension ( p2, 1 )
			...
			if vioGetElementData ( p1, "highNoonWin" ) > vioGetElementData ( p2, "highNoonWin" ) then
				vioSetElementData ( p1, "money", vioGetElementData ( p2, "money" ) + (summe*2) )
				outputChatBox ( "Du hast das Duell verloren! Viel Glueck beim naechsten mal!", p2, 125, 0, 0 )
				outputChatBox ( "Du hast das Duell gewonnen und erhaelst "..(summe*2).." $!", p1, 200, 200, 0 )
			elseif vioGetElementData ( p1, "highNoonWin" ) < vioGetElementData ( p2, "highNoonWin" ) then
				vioSetElementData ( p2, "money", vioGetElementData ( p2, "money" ) + (summe*2) )
				...
				vioSetElementData ( p2, "money", vioGetElementData ( p2, "money" ) + summe )
				vioSetElementData ( p1, "money", vioGetElementData ( p1, "money" ) + summe )
...
function endHighNoon ( p1, p2, summe )
	...
	if isElement ( p1 ) then
		if vioGetElementData ( p1, "isInHighNoon" ) then
	...
	elseif isElement ( p2 ) then
		if vioGetElementData ( p2, "isInHighNoon" ) then
			vioSetElementData ( p2, "highNoonWin", vioGetElementData ( p2, "highNoonWin" ) + 1 )
			endHighNoonForPlayer ( p2, true )
			setElementPosition ( p2, -2617.2189941406, 209.10488891602, 4.4970808029175 )
			setElementDimension ( p2, 1 )
...
function endHighNoonForPlayer ( player, bool )
	...
	takeAllWeapons ( player )
	setElementModel ( player, tonumber ( vioGetElementData ( player, "skinid" ) ) )
	setElementDimension ( player, 0 )
