...
function presidentGotWasted ( _, killer )
	...
	if isElement ( killer ) and getElementType ( killer ) == "player" then
		local frac = vioGetElementData ( killer, "fraktion" )
		...
		outputLog ( "Präsi-Konvoi - "..getPlayerName(killer).." hat Präsi getötet - "..fraktionNames[vioGetElementData(killer,"fraktion")], "aktion" )
	end
	setTimer ( function ( )
...
function startPresiKonvoi ( player )
	...
	outputLog ( "Präsi-Konvoi - "..fraktionNames[vioGetElementData(player,"fraktion")], "aktion" )
