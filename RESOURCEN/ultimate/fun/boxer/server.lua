...
addEventHandler("boxenstarten1", getRootElement(), function(player)
	...
	botErstellen( player )
	if not vioGetElementData(player, "boxlvl") then
		vioSetElementData(player, "boxlvl", 0)
	end
	if vioGetElementData(player, "boxlvl") == 2 then
...
function boxerLebtNochFrage( player ) --Diese Funktion prüft nach ob der Boxer noch lebt/der spieler gewonnen hat
	if isPedDead(boxer[player]) then
		...
		if vioGetElementData(player, "boxlvl") == 2 then
			...
			vioSetElementData(player, "money", vioGetElementData(player, "money") + 800)
			KampfstyledesBoxers[player] = nil
		elseif vioGetElementData(player, "boxlvl") == 1 then
			vioSetElementData(player, "boxlvl", 2)
			variable[player] = 155
		...
		elseif vioGetElementData(player, "boxlvl") == 0 then
			vioSetElementData(player, "boxlvl", 1)
			variable[player] = 230
