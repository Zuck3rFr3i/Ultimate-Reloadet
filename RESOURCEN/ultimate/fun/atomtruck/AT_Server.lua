...
function ATBasis ( player )
	if not getPedOccupiedVehicle ( player ) then
		if isOnDuty(player) then
			...
			if oldwanted <= 4 then
				setPlayerWantedLevel ( player, oldwanted + 2 )
				vioSetElementData ( player, "wanteds", oldwanted + 2 )
				outputChatBox ( "Du hast gerade 2 Wanteds für den Einbruchsversuch in einen gesperrten Bereich erhalten!", player, 255, 255, 0 )
				setElementPosition ( player, 2441.9123, -2121.296, 13.54 )
				setElementRotation ( player, 0, 0, 0 )
			elseif oldwanted == 5 then
				setPlayerWantedLevel ( player, oldwanted + 1 )
				vioSetElementData ( player, "wanteds", oldwanted + 1 )
				outputChatBox ( "Du hast gerade 1 Wanted für den Einbruchsversuch in einen gesperrten Bereich erhalten!", player, 255, 255, 0 )
				setElementPosition ( player, 2441.9123, -2121.296, 13.54 )
				setElementRotation ( player, 0, 0, 0 )
			else
				setElementPosition ( player, 2441.9123, -2121.296, 13.54 )
				setElementRotation ( player, 0, 0, 0 )
				outputChatBox ( "Einbruchversuch in einen gesperrten Bereich ist untersagt!", player, 255, 255, 0 )
			end
		end
	end	
end
					...
					if isOnDuty(player) then
						if getRealTime().hour >= 12 then
							if money >= ATKaution then
								vioSetElementData ( player, "money", money - ATKaution )
								ATTruck = createVehicle ( 515, 2545.5009765625, -2117.2802734375, 14.5575923919680, 0, 0, 90, "ATOM" )
								setVehicleColor ( ATTruck, 0, 0, 0 )
								setElementHealth ( ATTruck, 2000 )
								outputLog ( "Atomtruck - "..getPlayerName(player).." - "..fraktionNames[vioGetElementData ( player, "fraktion")], "aktion" )
...
function finishAT_func ( hitelement )
	if getElementType( hitelement ) == "vehicle" then
		local veh = hitelement
		local money = vioGetElementData ( driverScript, "money" )
		if ( hitElement == AtomTransporter ) then
			if AbgabeATCheck then
				...
				vioSetElementData ( driverScript, "money", money + ATKaution )
		...
		if ( vioGetElementData ( player, "adminlvl" ) >= 3 ) then
			...
			elseif stringWithAllParameters == "" or stringWithAllParameters == " " or stringWithAllParameters == "  " then
				...
				if ( ATRunning == true ) then
					...
					local money = vioGetElementData ( driverScript, "money" )
						vioSetElementData ( ATTruck, "Atom", false )
						destroyElement ( ATTruck )
						destroyElement ( ATTrailer )
						vioSetElementData ( driverScript, "money", money + ATKaution )
