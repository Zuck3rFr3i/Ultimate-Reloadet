...
function fill_memberlist_func ( )
	...
	for player, rang in pairs (fraktionMemberList) do
		...
		local Zeile = guiGridListAddRow ( gGrid["fraktion"] )
		local frakzeit = fraktionMemberListInvite[player] or "Unbekannt"
		guiGridListSetItemText ( gGrid["fraktion"], Zeile, Name, player, false, false )
		guiGridListSetItemText ( gGrid["fraktion"], Zeile, Rang, rang, false, false )	
		guiGridListSetItemText ( gGrid["fraktion"], Zeile, Zeit, frakzeit, false, false )
		guiGridListSetItemText ( gGrid["fraktion"], Zeile, Status, status, false, false )	
		if status == "online" then
			guiGridListSetItemColor ( gGrid["fraktion"], Zeile, Status, 0, 125, 0 )
		elseif status == "offline" then
			guiGridListSetItemColor ( gGrid["fraktion"], Zeile, Status, 125, 0, 0 )
		end
	end
end
