...
function banCheck ( nick, ip, uname, serial )
	...
	elseif string.find ( nick, "'" ) then
		...
		if playerUID[nick] then
			local result = dbPoll ( dbQuery ( handler, "SELECT ??, ??, ?? FROM ?? WHERE ??=?", "STime", "Grund", "AdminUID", "ban", "UID", playerUID[nick] ), -1 )
			if result and result[1] then
				local bantime = result[1]["STime"]
				bool = not ( bantime == 0 )
			end
			local cancome = true
				...
				if pwresult == hash ( "sha512", passwort ) then	
					...
					if result then
						if result[1] then
							...
							vioSetElementData ( player, "object", tonumber ( dbPoll ( dbQuery ( handler, "SELECT ?? FROM ?? WHERE ??=?", "Objekt", "inventar", "UID", playerUID[pname] ), -1 )[1]["Objekt"] ) )
