...
function createNewAI ()
	...
	while true do
		...
		elseif not usedAIPositions[rnd] then
			...
			vioSetElementData ( ped, "typ", typ )
			vioSetElementData ( ped, "clickPed", true )
			vioSetElementData ( ped, "pos", rnd )
			
			if typ == "wdealer" then
				local gun = generateWeapon()
				local price = math.floor ( aiGunPrices[gun] * math.random ( 7, 15 ) / 10 )
				vioSetElementData ( ped, "item", aiGuns[gun] )
				vioSetElementData ( ped, "price", price )
			elseif typ == "sdealer" then
				local rnd = math.random ( 10, 100 )
				local price = math.floor ( rnd * math.random ( 5, 15 ) / 10 * 40 )
				vioSetElementData ( ped, "item", rnd )
				vioSetElementData ( ped, "price", price )
			...
			elseif typ == "gunbuyer" then
				...
				if rnd == 1 then
					...
					vioSetElementData ( ped, "item", 22 )
					vioSetElementData ( ped, "price", price )
				elseif rnd == 2 then
					local price = math.random ( 75, 300 )
					vioSetElementData ( ped, "item", 4 )
					vioSetElementData ( ped, "price", price )
				else
					local price = math.random ( 250, 750 )
					vioSetElementData ( ped, "item", 30 )
					vioSetElementData ( ped, "price", price )
				end
			end
			...
			addEventHandler ( "onPedWasted", ped,
				function ( ammo, killer )
					...
					if killer then
						if vioGetElementData ( killer, "wanteds" ) then
							vioSetElementData ( killer, "wanteds", vioGetElementData ( killer, "wanteds" ) + 2 )
						else 
							vioSetElementData ( killer, "wanteds", 2 )
						end
...
function agreeWithPed_func ()
	...
	local player = source
	local ped = vioGetElementData ( player, "curclicked" )
	if isElement ( ped ) then
		if not isPedDead ( ped ) then
			local typ = vioGetElementData ( ped, "typ" )
			if typ == "car" then
				local car = curAiCarSpots[vioGetElementData ( ped, "id" )]["car"]
				buyCarFromAI ( player, car, ped )
			else
				local item = vioGetElementData ( ped, "item" )
				local price = vioGetElementData ( ped, "price" )
				if typ == "bum" then
					local sucess = false
					for i = 1, 3 do
						if vioGetElementData ( player, "food"..i ) == 3 then
							sucess = true
							vioSetElementData ( player, "food"..i, 0 )
							break
						end
					end
					if not sucess then
				...
				elseif typ == "wdealer" then
					...
					if money >= price then
						if vioGetElementData ( player, "gunlicense" ) == 0 then
				...
				elseif typ == "sdealer" then
					...
					if money >= price then
						...
						vioSetElementData ( player, "flowerseeds", vioGetElementData ( player, "flowerseeds" ) + item )
				...
				elseif typ == "dealer" then
					...
					if money >= price then
						vioSetElementData ( player, "money", money - price )
						vioSetElementData ( player, "drugs", vioGetElementData ( player, "drugs" ) + item )
